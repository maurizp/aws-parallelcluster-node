...
def validate_subprocess_argument(argument):
    # As for https://www.gnu.org/software/bash/manual/bash.html#Lists and for information provided by security
    forbidden_patterns = ["&", "|",  ";",  "$",  ">", "<", "`", "\\", "!", "#", "\n"]
    ...
    if any(map(_argument.__contains__, forbidden_patterns)):
        raise Exception(f"Value of provided argument contains at least a forbidden pattern")
    return True
